...
use crate::models::rsi_strategy::{
    Entity as RSIStrategyEntity,
    ActiveModel as RSIStrategyActiveModel,
    ExecutionEntity as RSIExecutionEntity,
    CreateRSIStrategyRequest, UpdateRSIStrategyRequest,
    RSIStrategyResponse, RSIStrategiesResponse, RSIExecutionResponse,
    RSIStatus,
};
...
pub async fn create_rsi_strategy(
    db: web::Data<DatabaseConnection>,
    req: HttpRequest,
    body: web::Json<CreateRSIStrategyRequest>,
    ...
    let existing_strategy = RSIStrategyEntity::find()
        .filter(crate::models::rsi_strategy::Column::UserId.eq(user_id))
        .filter(crate::models::rsi_strategy::Column::Name.eq(&body.name))
        .one(db.get_ref())
        .await
    ...
    // Validate RSIStrategyConfig
    body.config.validate().map_err(|e| AppError::BadRequest(format!("Invalid RSIStrategyConfig: {}", e)))?;

    // Serialize RSIStrategyConfig to JSON
    let config_json = serde_json::to_string(&body.config)
        .map_err(|e| AppError::BadRequest(format!("Failed to serialize RSIStrategyConfig: {}", e)))?;

    // Create the strategy
    let new_strategy = RSIStrategyActiveModel {
        id: Set(Uuid::new_v4()),
        user_id: Set(user_id),
        name: Set(body.name.clone()),
        asset_symbol: Set(body.asset_symbol.to_uppercase()),
        status: Set(RSIStatus::Active.into()),
    ...
    let response = RSIStrategyResponse {
        id: strategy.id,
        user_id: strategy.user_id,
        name: strategy.name.clone(),
        asset_symbol: strategy.asset_symbol.clone(),
        status: strategy.status.clone(),
        config: strategy.get_rsi_config().map_err(|e| AppError::BadRequest(e))?,
        ...
        last_rsi_value: strategy.last_rsi_value,
...
/// Get user's RSI strategies
pub async fn get_rsi_strategies(
    db: web::Data<DatabaseConnection>,
    ...
    let strategies = RSIStrategyEntity::find()
        .filter(crate::models::rsi_strategy::Column::UserId.eq(user_id))
        .order_by_desc(crate::models::rsi_strategy::Column::CreatedAt)
        .all(db.get_ref())
        .await
        .map_err(AppError::DatabaseError)?;
    ...
    for strategy in strategies {
        ...
        let recent_executions = RSIExecutionEntity::find()
            .filter(crate::models::rsi_strategy::execution::Column::StrategyId.eq(strategy.id))
            .order_by_desc(crate::models::rsi_strategy::execution::Column::ExecutionTimestamp)
            .limit(10)
            .all(db.get_ref())
            .await
            .map_err(AppError::DatabaseError)?;

        let execution_responses: Vec<RSIExecutionResponse> = recent_executions.into_iter()
            .map(|exec| RSIExecutionResponse {
                id: exec.id,
        ...
        if strategy.status == String::from(RSIStatus::Active) {
            active_strategies += 1;
        }
        total_win_rate += strategy.calculate_win_rate();

        let strategy_response = RSIStrategyResponse {
            id: strategy.id,
            user_id: strategy.user_id,
            name: strategy.name.clone(),
            asset_symbol: strategy.asset_symbol.clone(),
            status: strategy.status.clone(),
            config: strategy.get_rsi_config().unwrap_or_else(|_| Default::default()),
            total_invested: strategy.total_invested,
            total_purchased: strategy.total_purchased,
            average_buy_price: strategy.average_buy_price,
            current_position: strategy.current_position,
    ...
    let response = RSIStrategiesResponse {
...
/// Get a specific RSI strategy
pub async fn get_rsi_strategy(
    db: web::Data<DatabaseConnection>,
    market_service: web::Data<MarketDataService>,
    req: HttpRequest,
    path: web::Path<Uuid>,
    ...
    // Get the strategy
    let strategy = RSIStrategyEntity::find_by_id(strategy_id)
        .one(db.get_ref())
        .await
        .map_err(AppError::DatabaseError)?
...
/// Update an RSI strategy
pub async fn update_rsi_strategy(
    db: web::Data<DatabaseConnection>,
    req: HttpRequest,
    path: web::Path<Uuid>,
    body: web::Json<UpdateRSIStrategyRequest>,
) -> Result<HttpResponse, AppError> {
    let user_id = req.extensions()
        .get::<Uuid>()
        .copied()
    ...
    // Get the strategy
    let mut strategy: RSIStrategyActiveModel = RSIStrategyEntity::find_by_id(strategy_id)
        .one(db.get_ref())
        .await
        .map_err(AppError::DatabaseError)?
        .ok_or_else(|| AppError::NotFound("Strategy not found".to_string()))?
    ...
    if let Some(config) = &body.config {
        ...
        config.validate().map_err(|e| AppError::BadRequest(format!("Invalid RSIStrategyConfig: {}", e)))?;
    ...
    let response = RSIStrategyResponse {
        ...
        config: updated_strategy.get_rsi_config().unwrap_or_else(|_| Default::default()),
        ...
        last_rsi_value: updated_strategy.last_rsi_value,
...
/// Delete an RSI strategy
pub async fn delete_rsi_strategy(
    db: web::Data<DatabaseConnection>,
    req: HttpRequest,
    path: web::Path<Uuid>,
) -> Result<HttpResponse, AppError> {
    ...
    RSIStrategyEntity::delete_by_id(strategy_id)
        .exec(db.get_ref())
        .await
        .map_err(AppError::DatabaseError)?;
...
/// Get RSI strategy execution statistics
pub async fn get_rsi_execution_stats(
    db: web::Data<DatabaseConnection>,
    req: HttpRequest,
) -> Result<HttpResponse, AppError> {
    let user_id = req.extensions()
