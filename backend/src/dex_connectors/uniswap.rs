...
const WETH9: &str = "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2";

// Fee tiers for Uniswap V3 pools (in hundredths of a bip, i.e. 1e-6)
const FEE_LOW: u32 = 500; // 0.05%
const FEE_MEDIUM: u32 = 3000; // 0.3%
const FEE_HIGH: u32 = 10000; // 1%

/// Uniswap V3 connector for Ethereum
///
/// This connector supports:
/// - Token swaps via SwapRouter02
/// - Quote estimation via QuoterV2
/// - ETH and ERC20 balance queries
...
impl UniswapConnector {
    pub fn new(credentials: WalletCredentials) -> Result<Self, DexError> {
        // Get Ethereum RPC URL from environment or use default
    ...
    fn parse_private_key(private_key: &str) -> Result<LocalWallet, DexError> {
        ...
        if key_str.len() != 64 {
            return Err(DexError::InvalidCredentials(
                "Invalid Ethereum private key length (expected 64 hex characters)".to_string()
            ));
        }
    ...
    async fn get_eth_balance(&self, address: Address) -> Result<U256, DexError> {
        self.provider
            .get_balance(address, None)
            .await
            .map_err(|e| DexError::NetworkError(format!("Failed to get ETH balance: {}", e)))
    }

    async fn get_erc20_balance(&self, token_address: Address, wallet_address: Address) -> Result<U256, DexError> {
    ...
    async fn execute_swap_exact_input_single(
        ...
        let router_address = Address::from_str(SWAP_ROUTER_02)
            .map_err(|_| DexError::InternalError("Invalid router address".to_string()))?;

        // First, approve the router to spend tokens if it's not ETH
        let weth_address = Address::from_str(WETH9)
            .map_err(|_| DexError::InternalError("Invalid WETH address".to_string()))?;
        ...
        let tx = ethers::types::TransactionRequest {
            ...
            data: Some(calldata.into()),
            value: if token_in == weth_address { Some(amount_in) } else { None },
            ..Default::default()
        };
    ...
    async fn approve_token(&self, token: Address, spender: Address, amount: U256) -> Result<(), DexError> {
        // ERC20 approve(address,uint256) function
...
#[async_trait]
impl DexConnector for UniswapConnector {
    async fn test_connection(&self) -> Result<bool, DexError> {
        tracing::info!("Testing Uniswap connection for wallet: {}", self.credentials.wallet_address);

        match self.provider.get_block_number().await {
            Ok(block_number) => {
                tracing::info!("Connected to Ethereum. Current block: {}", block_number);
                Ok(true)
            }
            Err(e) => {
                tracing::error!("Uniswap connection test failed: {:?}", e);
    ...
    async fn get_wallet_balance(&self) -> Result<WalletBalance, DexError> {
        tracing::info!("Fetching Uniswap wallet balance for: {}", self.credentials.wallet_address);
        ...
        // Get ETH balance
        let eth_wei = self.get_eth_balance(address).await?;
        let eth_balance = Self::wei_to_decimal(eth_wei, 18);

        // TODO: Query all ERC20 token balances
    ...
    async fn get_token_balance(&self, token_address: &str) -> Result<TokenBalance, DexError> {
        tracing::info!("Fetching ERC20 token balance for: {}", token_address);
        ...
        let balance_wei = self.get_erc20_balance(token, wallet).await?;
    ...
    async fn get_swap_quote(
        ...
        tracing::info!("Getting Uniswap swap quote: {} {} -> {}", amount, from_token, to_token);
    ...
    async fn execute_swap(
        ...
        tracing::info!(
            "Executing Uniswap swap: {} {} -> {} (slippage: {}%)",
        ...
        // Execute swap with medium fee tier (most common)
    ...
    async fn get_pool_info(
        ...
        tracing::info!("Fetching Uniswap pool info: {} / {}", token0, token1);
    ...
    async fn add_liquidity(
        ...
        tracing::info!(
            "Adding liquidity to Uniswap: {} {} / {} {} (slippage: {}%)",
    ...
    async fn remove_liquidity(
        ...
        tracing::info!(
            "Removing liquidity from Uniswap: {} from {} / {} (slippage: {}%)",
    ...
    async fn get_transaction_status(&self, tx_hash: &str) -> Result<TransactionStatus, DexError> {
        tracing::info!("Checking Ethereum transaction status: {}", tx_hash);
    ...
    async fn get_gas_price(&self) -> Result<String, DexError> {
        tracing::info!("Fetching current Ethereum gas price");
